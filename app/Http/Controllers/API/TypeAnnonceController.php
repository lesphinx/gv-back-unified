...
class TypeAnnonceController extends Controller
{
    ...
    public function index()
    {
                ...
                //   fetchLog(TypeAnnonce::class);
                    return response()->json(['content'=>TypeAnnonce::with('positions')->orderBy('created_at','desc')->paginate(10),'message'=>'liste des TypeAnnonces'],200,['Content-Type'=>'application/json']);
    ...
    public function store(Request $request)
    {
       $data = [
        ...
        $new_Tannonce=TypeAnnonce::create($data);
        $var_Tannonce = $new_Tannonce->id;
        ...
        if($request->positionId!= null)
        {
                    ...
                    $data_tarif_a = [
                        'prix_caractere' => $request->prix,
                        'nbre_caractere' => $request->nbrecaractere,
                        'devise' => $request->devise,
                        'prix_image' => $request->priximage,
                        'position' => $request->positionId,
                        'type_annonce' => $var_Tannonce,
                        'slug'=>str_slug(name_generator('tarif_annonce',10),'-')
                    ];
                    //dd($data_tarif_a);
                    TarifAnnonce::create($data_tarif_a);
    ...
    public function destroy(Request $request,$slug)
     {
              if (TypeAnnonce::where('slug','=',$slug)->first()){
                    $typeannonce = TypeAnnonce::where('slug','=',$slug)->first();
                    $typeannonce->delete();
                    deleteLog(TypeAnnonce::class,setZero());
