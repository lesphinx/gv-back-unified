<template>
    <div class="row">
        ...
        <template v-if="items == null" class="row">
            <b-alert show variant="danger">Pas province enregistré pour le moment</b-alert>
        </template>
    </div>
    ...
    export default {
        ...
        methods: {
            ...
            _delete(item) {
                ...
                Swal.fire({
                    title: 'Confirmer la suppression de la province ' + _item.nom,
                    ...
                    if (result.value) {
                        axios.delete('/api/province/' + _slug)
                            .then(function (response) {
                            }).catch(function (response) {
                        });
                        this._fetch('/api/province');
                        Swal.fire(
                            'Succès !',
                            'La province a été supprimé avec succès!',
                            'success'
                        )
            ...
            onUpdate(province) {
                this.$root.$emit('update-province', province );
            },

            onDetail(id){

                var _nom = province.nom;
                var _desc = province.description;

                Swal.fire({
                    title: '<strong>Détail province</strong>',
                    html:
                        '<p><u>Nom :</u> ' + _nom +' </p>'
                     ,
